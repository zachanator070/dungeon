...
function doGo(tokens) {
    ...
    if (tokens.length !== 2) {
        ...
        // delete GO token
        tokens.splice(0, 1);
        const direction = tokens[0].value;
...
function doUse(tokens) {
    ...
    if (tokens.length === 2) {
        ...
        const name1 = tokens[1].value;
        const item1 = findItem(name1);

        if (item1 !== null) {

            const name2 = tokens[3].value;
            const item2 = findItem(name2);

            if (item2 !== null) {
                if (tokens[0].value.toLowerCase() === "attack") {
                    const result = item2.use(item1);
                    addNewLine(result);
                } else {
                    const result = item1.use(item2);
                    addNewLine(result);
                }
            } else {
...
function showHelp(tokens) {
    if (tokens.length === 1) {
        ...
        if (token.type === 'GO') {
            ...
            addNewLine("You are already using the help command, it looks like you don\'t need any more help.");
        } else {
